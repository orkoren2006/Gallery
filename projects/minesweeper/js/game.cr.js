...
// @CR hey or , i just looked into your project :)
// first , your code is very clean and understandable , love that you make notes near the code for explain your moves.
// the function names and variables names are correct and strong ! love that allso.
// few small things , when you check boolean var in a condition (if) the right syntax for this is ==>
// if(cell.isMine === true) === if(cell.isMine) 
// allso we want to render from JS less that we can . if we have a static html we prefare to put in the DOM and not in the JS.
// when we done with project. clean the old code. it make your code look better and less loaded.
// i left a few notes in the code you can take a look :)
// sum :  your proj looks great and the code allso  . love the notes and that evreything organized. keep it going !
var MINE = '💣';
var FLAG = '🚩';
var NEUTRAL = '😐'
var HAPPY = '😀';
...
function renderBoard(board) {
    // @CR the renderBoard should only render the board , the BULB`S render is a static elements and you should
    // put them in the DOM , we render throught the JS only dynamic elements.
    // after click you can change the "this" attr and change bgc // img // or put in hidden.
...
function cellClicked(elCell, i, j) {
    ...
    // @CR we prefer to make a "GUARD" . 
    // use if(!gGame.isOn) return.
    // more readabble and understanting for the next programmer that look into your code. make them exactly same
    // thing but more readabble.
    if (gGame.isOn) {
        // if (gBoard[i][j].isMine === true && gGame.showCount === 0) { //attempt for first click not mine
        //     renderBoard(gBoard);
        // } else {
        if (isHint === true) {
            // @CR never use a varible check in if like that.
            // if(isHint === true) is exact the same like || if(isHint)
            revealNegs(i, j);
            isHint = false;
        } else {
            stop();
            startStopwatch();
            var cell = gBoard[i][j]
            //@CR allso in short if .. the right syntax is and if we do '=' the right synyax for this is
            // cell.isShown = !cell.isShown ? true : false
                    ...
                    for (var i = 0; i < gBoard.length; i++) {
                        for (var j = 0; j < gBoard.length; j++) { //itirates through all the other mines in board to reveal them as well
                            if (gBoard[i][j].isMine === true && gBoard[i][j] !== cell) {
                                // @CR allso here gBoard[i][j].isMine && gBoard[i][j] !== cell
...
function hideNegs(idxI, idxJ) { //seems like the overwrite bug is completely fixed 
    setTimeout(function () {
        var index = gBoard[idxI][idxJ];
